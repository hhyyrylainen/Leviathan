...
int main(int argcount, char* args[]){
    ...
    if(Leviathan::GlobalCEFHandler::CEFFirstCheckChildProcess(argcount, args, Return, KeepThisSafe
        #ifdef _WIN32
            , hInstance
        #endif
        ))
    ...
    Logger mainLog("Demos" + std::string("Log.txt"));

    // World creator object //
    GameWorldFactory worldFactory;
    
    // Create program object //
    DemosApplication app;

    std::unique_ptr<AppDef> ProgramDefinition(AppDef::GenerateAppdefine(
            "./EngineConf.conf", "./Demos.conf", "./DemosKeys.conf",
            &DemosApplication::CheckGameConfigurationVariables, &DemosApplication::CheckGameKeyConfigVariables));
    ...
    ProgramDefinition->SetMasterServerParameters(MasterServerInformation()).
        ...
        "Leviathan Demos version " GAME_VERSIONS, "Demos",
        "GAME_VERSIONS");
    
    // Create window last //
    ProgramDefinition->StoreWindowDetails(DemosApplication::GenerateWindowTitle(), true,
#ifdef _WIN32
