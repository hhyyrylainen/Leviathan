...
#include "../../ThirdParty/angelscript/sdk/angelscript/source/as_memory.h"
...
void stdcall allocScriptObject(asCObjectType* type, asCScriptFunction* constructor,
    asIScriptEngine* engine, asSVMRegisters* registers);
...
void stdcall engineDestroyFree(
    asCScriptEngine* engine, void* memory, asCScriptFunction* destruct);

void stdcall engineFree(asCScriptEngine* engine, void* memory);

void stdcall engineCallMethod(
    asCScriptEngine* engine, void* object, asCScriptFunction* method);
...
// Wrapper functions to cast between types, or perform math on large types, where doing so is
// overly complicated in the ASM
#ifdef _MSC_VER
...
size_t stdcall div_ull(
    unsigned long long* div, unsigned long long* by, unsigned long long* result)
{
    if(*by == 0)
        return 1;
    *result = *div / *by;
...
struct SystemCall {
    ...
    SystemCall(Processor& CPU, FloatingPointUnit& FPU,
        std::function<void(JumpType, bool)> ConditionalReturn, asDWORD* const& bytecode,
        unsigned JitFlags) :
        cpu(CPU),
        fpu(FPU), returnHandler(ConditionalReturn), pOp(bytecode), flags(0)
    {
    ...
    void callSystemFunction(
        asCScriptFunction* func, Register* objPointer = 0, unsigned callFlags = 0);
...
private:
    ...
    void call_cdecl_obj(asSSystemFunctionInterface* func, asCScriptFunction* sFunc,
        Register* objPointer, bool last);
    void call_thiscall(
        asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer);
    void call_simple(Register& objPointer, asCScriptFunction* func);

    void call_64conv(asSSystemFunctionInterface* func, asCScriptFunction* sFunc,
        Register* objPointer, ObjectPosition pos);
...
// Copy of value register in 32 bit mode when returns are being ignored, not used alongside
// retPointer
...
int asCJITCompiler::CompileFunction(asIScriptFunction* function, asJITFunction* output)
{
    ...
    // Do a first pass through the bytecode to mark all locations we are going to be jumping
    // to, that way we can prevent running multi-op optimizations on them.
    asDWORD* passOp = pOp;
    while(passOp < end) {
    ...
    // Get the active page, or create a new one if the current one is missing or too small (256
    // bytes for the entry and a few ops)
    ...
    *output = activePage->getFunctionPointer<asJITFunction>();
    ...
    // If we are outside of opcodes we can execute, ignore all ops until a new JIT entry is
    // found
    ...
    Register pax(cpu, EAX, pBits), pbx(cpu, EBX, pBits), pcx(cpu, ECX, pBits),
        pdx(cpu, EDX, pBits), esp(cpu, ESP, pBits), pdi(cpu, R12, pBits), esi(cpu, R13, pBits);
    ...
    // Pointer-sized registers
    Register pax(cpu, EAX, pBits), pbx(cpu, EBX, pBits), pcx(cpu, ECX, pBits),
        pdx(cpu, EDX, pBits), esp(cpu, ESP, pBits), pdi(cpu, EDI, pBits), esi(cpu, ESI, pBits);
    Register rarg(cpu, EDX, pBits);
#endif

    // JIT FUNCTION ENTRY
    //==================
    // Push unmutable registers (these registers must retain their value after we leave our
    // function)
    cpu.push(esi);
    cpu.push(edi);
    cpu.push(ebx);
    ...
    as<void*>(*ebp + offsetof(asSVMRegisters, programPointer)) =
        rarg; // Set the bytecode pointer based on our exit
    as<void*>(*ebp + offsetof(asSVMRegisters, stackFramePointer)) =
        pdi; // Return the frame pointer
    ...
    auto ReturnFromScriptCall = [&]() {
        if(script_ret) {
            ...
            script_ret = cpu.op;
            // The VM Registers are already in the correct state, so just do a simple return
            // here
            esp += functionReserveSpace;
            cpu.pop(ebp);
            cpu.pop(ebx);
    ...
    auto JitScriptCall = [&](asCScriptFunction* func) {
        ...
        if(entryPoint && func->scriptData->jitFunction) {
            ...
            deferredPointers.insert(
                std::pair<asIScriptFunction*, DeferredCodePointer>(func, def));
        }
    ...
    auto ReturnFromJittedScriptCall = [&](void* expectedPC) {
        ...
        pbx =
            as<void*>(*ebp + offsetof(asSVMRegisters, valueRegister)); // update value register
    };
    ...
    auto do_jump = [&](JumpType type) {
        ...
        if(bc > pOp) {
            ...
            // We can't handle this address, so generate a special return that does the jump
            // ahead of time
            rarg = bc;
    ...
    auto check_space = [&](unsigned bytes) {
        ...
        if(remaining < bytes + cpu.jumpSpace) {
            CodePage* newPage =
                new CodePage(codePageSize, ((char*)activePage->page + activePage->size));
    ...
    while(pOp < end) {
        ...
        if(activeSwitch) {
            activeSwitch->buffer[int(activeSwitch->count) - int(activeSwitch->remaining)] =
                (unsigned char*)cpu.op;
        ...
        *pdx = (void*)beg;
        ...
        // Multi-op optimization - special cases where specific sets of ops serve a common
        // purpose
        ...
        if(pNextOp < end && jumpTable[pNextOp - start] == nullptr) {
            ...
            case asBC_PshVPtr:
                if(reservedPushBytes == 0 && nextOp == asBC_COPY) {
                    ...
                    cpu.call_cdecl((void*)memcpy,"rrc", &arg0, &pax,
unsigned(asBC_WORDARG0(pNextOp))*4); void* skip_ret = cpu.prep_short_jump(Jump);
                    //ERR
    ...
    reservedPushBytes -= use;
        ...
        case asBC_JMPP:
            if((flags & JIT_NO_SWITCHES) == 0) {
                unsigned cases = 1;
                {
                    // This information isn't stored for us to recover, so we rely on the
                    // format of switch cases Each one is a series of asBC_JMP ops and a single
                    // remaining case at the end (default)
        ...
        case asBC_STR:
            {
                const asCString &str =
        ((asCScriptEngine*)function->GetEngine())->GetConstantString(asBC_WORDARG0(pOp)); esi
        -= sizeof(void*) + sizeof(asDWORD); as<void*>(*esi + sizeof(asDWORD)) =
        (void*)str.AddressOf(); as<asDWORD>(*esi) = (asDWORD)str.GetLength(); } break;*/
        case asBC_CALLSYS:
        case asBC_Thiscall1: {
            check_space(512);
            asCScriptFunction* func =
        ...
        case asBC_CALLBND: {
            ...
            if(flags & JIT_NO_SCRIPT_CALLS) {
                ...
                cpu.call_stdcall(
                    (void*)callBoundFunction, "rc", &arg0, (unsigned int)asBC_INTARG(pOp));
                pax &= pax;
        ...
        case asBC_ALLOC: {
            ...
            if(objType->flags & asOBJ_SCRIPT_OBJECT) {
                ...
                cpu.call_stdcall((void*)engineAlloc, "pp",
                    (asCScriptEngine*)function->GetEngine(), objType);

                if(func) {
                    as<void*>(*esp + local::allocMem) = pax;
                    auto pFunc =
                        (asCScriptFunction*)function->GetEngine()->GetFunctionById(func);
        ...
        case asBC_FREE: {
            ...
            if(!(objType->flags & asOBJ_REF) ||
                !(objType->flags &
                    asOBJ_NOCOUNT)) { // Only do FREE on non-reference types, or reference
                                      // types without fake reference counting
                check_space(128);
                ...
                if(beh->release) {
                    ...
                    asCScriptFunction* func =
                        (asCScriptFunction*)function->GetEngine()->GetFunctionById(
                            beh->release);
                    sysCall.callSystemFunction(func, &arg1, callFlags);
                } else if(beh->destruct) {
                    // Copy over registers to the vm in case the called functions observe the
                    // call stack
                    as<void*>(*ebp + offsetof(asSVMRegisters, programPointer)) = pOp;
                    as<void*>(*ebp + offsetof(asSVMRegisters, stackPointer)) = esi;

                    cpu.call_stdcall((void*)engineDestroyFree, "prp",
                        (asCScriptEngine*)function->GetEngine(), &arg1,
                        (asCScriptFunction*)function->GetEngine()->GetFunctionById(
                            beh->destruct));
                    ...
                    cpu.call_stdcall((void*)engineListFree, "ppr",
                        (asCScriptEngine*)function->GetEngine(), objType, &arg1);
                } else {
                    // Copy over registers to the vm in case the called functions observe the
                    // call stack
        ...
        case asBC_STOREOBJ: {
            ...
            pcx ^= pcx;
            (*edi - offset0)
                .direct_copy((*ebp + offsetof(asSVMRegisters, objectRegister)), eax);
        ...
        case asBC_REFCPY: {
            ...
            if(objType->flags & asOBJ_NOCOUNT) {
                ...
                check_space(512);
                // Copy over registers to the vm in case the called functions observe the call
                // stack
                as<void*>(*ebp + offsetof(asSVMRegisters, programPointer)) = pOp;
                as<void*>(*ebp + offsetof(asSVMRegisters, stackPointer)) = esi;
        ...
        case asBC_Cast: {
            ...
            asCObjectType* to = ((asCScriptEngine*)function->GetEngine())
                                    ->GetObjectTypeFromTypeId(asBC_DWORDARG(pOp));
...
#define cast(f, t)                                                   \
    {                                                                \
        ...
        case asBC_ClrHi:
            // Due to the way logic is handled, the upper bytes area always ignored, and don't
            // need to be cleared ebx &= 0x000000ff;
            break;
        case asBC_CallPtr: {
#ifdef JIT_64
            Register arg0 = as<void*>(cpu.intArg64(0, 0));
    ...
    asIScriptEngine* engine, asSVMRegisters* registers)
{
    ...
    void** dest =
        *(void***)(registers->stackPointer + constructor->GetSpaceNeededForArguments());
    if(dest)
        *dest = mem;
...
asCScriptFunction* stdcall callBoundFunction(asIScriptContext* ctx, unsigned short fid)
{
    ...
    // Imported functions can be bound to non-script functions; we just exit to the vm to
...
void SystemCall::callSystemFunction(
    asCScriptFunction* func, Register* objPointer, unsigned callFlags)
{
    callFlags |= flags;
...
#endif
    {
        ...
        // Note: We do not know parameter sizes for template factories, so we cannot compile
        // them However, they all receive a magic int& that we can detect (paramAutoHandles is
        // not empty, paramSize is)
#ifdef JIT_PRINT_UNHANDLED_CALLS
        unhandled();
#endif
...
#ifdef JIT_64
void SystemCall::call_64conv(asSSystemFunctionInterface* func, asCScriptFunction* sFunc,
    Register* objPointer, ObjectPosition pos)
{
    ...
    bool isVirtual = func->callConv == ICC_VIRTUAL_THISCALL ||
                     func->callConv == ICC_VIRTUAL_THISCALL_RETURNINMEM;
    ...
    if(sFunc->returnType.IsObject() && !sFunc->returnType.IsReference()) {
        if(sFunc->returnType.IsObjectHandle()) {
            ...
            as<void*>(*ebp + offsetof(asSVMRegisters, objectType)) =
                sFunc->returnType.GetTypeInfo();
            ...
            if(func->returnAutoHandle) {
                ...
                asCScriptFunction* addrefFunc =
                    (asCScriptFunction*)sFunc->GetEngine()->GetFunctionById(addref);

                cpu.call_stdcall(
                    (void*)engineCallMethod, "prp", sFunc->GetEngine(), &ret, addrefFunc);
            ...
            if(acceptReturn) {
                ...
                // However, anything that tries to read this when it isn't valid is making a
                // mistake as<void*>(*ebp + offsetof(asSVMRegisters,objectRegister)) = nullptr;
                ...
                if(destruct > 0) {
                    ...
                    cpu.call_stdcall((void*)engineCallMethod, "prp", sFunc->GetEngine(), &temp,
                        destructFunc);

                    cpu.end_short_jump(noError);
                }
            } else if(sFunc->returnType.GetBehaviour()->destruct > 0) {
                throw "Destructible returns not permitted here."; // Reference counting and
                                                                  // deletion
            }
        }
...
{
    Register eax(cpu, EAX), ecx(cpu, ECX), ebx(cpu, EBX), edx(cpu, EDX), ebp(cpu, EBP),
        esp(cpu, ESP);
    ...
    if(sFunc->returnType.IsObject() && !sFunc->returnType.IsReference()) {
        if(sFunc->returnType.IsObjectHandle()) {
            if(!acceptReturn) {
                if(func->returnAutoHandle)
                    throw "Auto handle returns not permitted here."; // Reference counting and
                                                                     // deletion
                return;
            }
            ...
            if(func->returnAutoHandle) {
                ...
                cpu.call_stdcall(
                    (void*)engineCallMethod, "prp", sFunc->GetEngine(), &eax, addrefFunc);
            ...
            if(!acceptReturn) {
                if(sFunc->DoesReturnOnStack() &&
                    sFunc->returnType.GetBehaviour()->destruct > 0)
                    throw "Destructible returns not permitted here."; // Reference counting and
                                                                      // deletion
                return;
            }
            ...
            if(sFunc->DoesReturnOnStack()) {
                ...
                if(destruct > 0) {
                    ...
                    cpu.call_stdcall((void*)engineCallMethod, "prp", sFunc->GetEngine(), &ecx,
                        destructFunc);
...
void SystemCall::call_cdecl_obj(asSSystemFunctionInterface* func, asCScriptFunction* sFunc,
    Register* objPointer, bool last)
{
...
#ifndef JIT_64
    // If we are not accepting returns, we have to save the value register as the call may
    // change the register
    if(!acceptReturn) {
...
void SystemCall::call_viaAS(asCScriptFunction* func, Register* objPointer)
{
...
#ifndef JIT_64
    // If we are not accepting returns, we have to save the value register or AngelScript will
    // change it regardless of the return type
    if(!acceptReturn) {
        pax = as<int>(*ebp + offsetof(asSVMRegisters, valueRegister) + 4);
        as<int>(*esp + local::regCopy) = pax;
    }
    ...
    if(acceptReturn) {
        if(((func->sysFuncIntf->hostReturnSize >= 1 &&
                !func->sysFuncIntf->hostReturnInMemory) ||
               isGeneric) &&
...
void stdcall engineSimpleMethod(
    asCScriptEngine* engine, void* obj, asSSystemFunctionInterface* i, asCScriptFunction* func)
{
...
void SystemCall::call_simple(Register& objPointer, asCScriptFunction* func)
{
    ...
    cpu.call_stdcall((void*)engineSimpleMethod, "prpp", (asCScriptEngine*)func->GetEngine(),
        &objPointer, sys, func);
}
