...
enum JITSettings {
    // Should the JIT attempt to suspend? (Slightly faster, but makes suspension very rare if
    // it occurs at all)
    JIT_NO_SUSPEND = 0x01,
    // Should the JIT reset the FPU entering System calls? (Slightly faster, may not work on
    // all platforms)
    JIT_SYSCALL_FPU_NORESET = 0x02,
    // Should the JIT support error events from System calls? (Faster, but exceptions will
    // generally be ignored, possibly leading to crashes)
    JIT_SYSCALL_NO_ERRORS = 0x04,
    // Do allocation/deallocation functions inspect the script context? (Faster, but won't work
    // correctly if you try to get information about the script system during allocations)
    ...
    // Reference counting functions which access the script context will produce undefined
    // results
