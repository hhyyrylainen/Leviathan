...
struct CodePage {
    ...
    // Marks bytes as used;
    // future calls to getFunctionPointer() will not reference the location that is being
    // marked as used
    ...
    // Returns the smallest page (in bytes) that can be allocated by a code page (Sizes other
    // than multiples of this size allocate an extra page)
...
// To work with the processor, create a set of 'Register' instances, each taking the RegCode of
// the associated register (e.g. Register eax(cpu, EAX))
...
struct Processor {
    ...
    // e.g. 32 bits for x86, indicating that operations should treat addresses as if they were
    // unsigned integers
    ...
    // Creates a jump to the new code page, and marks the current address as used on the old
    // code page Updates output pointer to the new code page's active page
    ...
    // args is a string like "rrcmrm" which specifies arguments as sourced by a Register*,
    // MemAddres*, or a constant EBP is invalid during the call
    ...
    // Pushes a constant value onto the stack (Pushes are always pushSize() large, values
    // beyond this size are an error)
    ...
    // Pushes the memory at <address> onto the stack (Pushes are always pushSize() large,
    // pushing larger values invokes multiple pushes)
    void push(MemAddress address);
    // Pops the value on the stack to the memory at <address> (Pops are always pushSize()
    // large, popping larger values invokes multiple pops)
    ...
    // Decrements ecx and jumps if it becomes 0; Optionally conditionally jumps based on a
    // Zero/NotZero test
    void loop(volatile byte* dest, JumpType type = Jump);

    // Copies from *esi to *edi, and adjusts them both by the data size according to the
    // direction flag
...
//  Also provides the means to generate MemAddresses relative to a register via dereference
//  (e.g. *eax+8)
// Implementation in virtual_asm_<processor instruction set>.cpp (e.g. virtual_asm_x86.cpp)
struct Register {
    Processor& cpu;
