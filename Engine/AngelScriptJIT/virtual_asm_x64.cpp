    ...
    if(addr.scaleFactor != 0) {
        ...
        if(addr.offset == 0 && addr.code % 8 != EBP)
            ...
            return *this << mod_rm(addr.other % 8, ADR8, SIB)
                         << sib(addr.scaleReg % 8, scale, addr.code % 8) << (char)addr.offset;
        else
            return *this << mod_rm(addr.other % 8, ADR32, SIB)
                         << sib(addr.scaleReg % 8, scale, addr.code % 8) << addr.offset;
    }

    if(addr.offset == 0 &&
        addr.code % 8 != EBP) { //[EBP] means absolute address, so we use [EBP+0] instead
...
void Processor::end_long_jump(void* p)
{
    ...
    if(offset < (int64_t)INT_MIN || offset > (int64_t)INT_MAX) {
        if(isSamePage)
            throw "Inside-page long jump too long. This should never ever happen, somebody "
                  "screwed the pooch.";
...
void Processor::jump(JumpType type, volatile byte* dest)
{
    ...
    if(offset >= CHAR_MIN && offset <= CHAR_MAX)
        ...
        *this << longJumpCodes[Jump] << (int)(offset - 3); // Long jump is 3 bytes larger, jump
                                                           // is from the end of the full opcode
    else
        *this << '\x0F' << longJumpCodes[type]
              << (int)(offset - 4); // Conditional long jump is 4 bytes larger, jump is from
                                    // the end of the full opcode
}
...
MemAddress::MemAddress(Processor& CPU, void* address) :
    ...
    bitMode(cpu.bitMode), Float(false), Signed(false), scaleFactor(0), scaleReg(NONE)
{}
