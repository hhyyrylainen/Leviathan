...
{
    switch(scale) {
    case 1: return Scale1; break;
    case 2: return Scale2; break;
    case 4: return Scale4; break;
    case 8: return Scale8; break;
    default: throw 0;
    }
}
...
Processor& Processor::operator<<(MemAddress addr)
{
    if(addr.absolute_address != 0) {
        if(addr.scaleFactor == 0)
            ...
            return *this << mod_rm(addr.other, ADR, SIB) << sib(addr.scaleReg, scale, EBP)
    ...
    if(addr.offset == 0 &&
        addr.code != EBP) { //[EBP] means absolute address, so we use [EBP+0] instead
...
void Processor::push(MemAddress address)
{
    ...
    case 8:
    case 16:
    case 32: *this << '\xFF' << address; break;
    case 64:
...
void Processor::pop(MemAddress address)
{
    ...
    case 8:
    case 16:
    case 32: *this << '\x8F' << address; break;
    case 64:
        *this << '\x8F' << address;
...
unsigned char shortJumpCodes[JumpTypeCount] = {0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77,
    0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 0x7F, 0xEB};
unsigned char longJumpCodes[JumpTypeCount] = {0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87,
    0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0xE9};
...
void Processor::jump(JumpType type, volatile byte* dest)
{
    ...
    if(offset >= CHAR_MIN && offset <= CHAR_MAX)
        ...
        *this << longJumpCodes[Jump]
              << offset -
                     3; // Long jump is 3 bytes larger, jump is from the end of the full opcode
    else
        *this << '\x0F' << longJumpCodes[type]
              << offset - 4; // Conditional long jump is 4 bytes larger, jump is from the end
                             // of the full opcode
}
...
{
    switch(bitMode) {
    case 8: cpu << '\xFE' << *this; break;
    case 16: cpu << '\x66' << '\xFF' << *this; break;
    case 32: cpu << '\xFF' << *this; break;
    case 64:
        cpu << '\xFF' << *this;
...
void MemAddress::operator-()
{
    other = EX_3;
    switch(bitMode) {
    case 8: cpu << '\xF6' << *this; break;
    case 16: cpu << '\x66' << '\xF7' << *this; break;
    case 32: cpu << '\xF7' << *this; break;
    case 64:
        ~MemAddress(*this);
        ++MemAddress(*this);
        break;
...
{
    switch(bitMode) {
    case 8: cpu << '\xC6' << *this << (byte)value; break;
    case 16: cpu << '\x66' << '\xC7' << *this << (unsigned short)value; break;
    case 32: cpu << '\xC7' << *this << value; break;
    }
}
...
void MemAddress::operator|=(unsigned int value)
{
    other = EX_7;
    switch(bitMode) {
    case 8: cpu << '\x80' << *this << (byte)value; break;
    case 16: cpu << '\x66' << '\x81' << *this << (unsigned short)value; break;
    case 32: cpu << '\x81' << *this << value; break;
    }
}
...
void MemAddress::operator=(Register fromReg)
{
    other = fromReg.code;
    switch(bitMode) {
    case 8: cpu << '\x88' << *this; break;
    case 16: cpu << '\x66' << '\x89' << *this; break;
    case 32: cpu << '\x89' << *this; break;
    case 64: throw 0;
    }
}
...
void Register::swap(MemAddress address)
{
    ...
    case 8: cpu << '\x86' << address; break;
    case 16: cpu << '\x66' << '\x87' << address; break;
    case 32: cpu << '\x87' << address; break;
    case 64: throw 0;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\xFE' << mod_rm(EX_1, REG, code); break;
    case 16: cpu << '\x66' << byte('\x48' + code); break;
    case 32: cpu << byte('\x48' + code); break;
    case 64: throw 0;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\xFE' << mod_rm(EX_0, REG, code); break;
    case 16: cpu << '\x66' << byte('\x40' + code); break;
    case 32: cpu << byte('\x40' + code); break;
    case 64: throw 0;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\x80' << mod_rm(EX_4, REG, code) << (byte)mask; break;
    case 16: cpu << '\x66' << '\x81' << mod_rm(EX_4, REG, code) << (unsigned short)mask; break;
    case 32:
    case 64:
        if(code == EAX)
...
{
    switch(getBitMode()) {
    case 8: cpu << '\x20' << mod_rm(other.code, REG, code); break;
    case 16: cpu << '\x66' << '\x21' << mod_rm(other.code, REG, code); break;
    case 32:
    case 64: cpu << '\x21' << mod_rm(other.code, REG, code); break;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\x80' << mod_rm(EX_1, REG, code) << (byte)mask; break;
    case 16: cpu << '\x66' << '\x81' << mod_rm(EX_1, REG, code) << (unsigned short)mask; break;
    case 32:
    case 64:
        if(code == EAX)
...
{
    switch(getBitMode()) {
    case 8: cpu << (byte)('\xB0' + code) << (byte)value; break;
    case 16: cpu << '\x66' << (byte)('\xB8' + code) << (unsigned short)value; break;
    case 32: cpu << (byte)('\xB8' + code) << (unsigned int)value; break;
    case 64: throw 0;
    }
}
...
void Register::copy_expanding(MemAddress address)
{
    address.other = code;
    switch(getBitMode(address)) {
    case 8: cpu << '\x0F' << '\xBE' << address; break;
    case 16: cpu << '\x0F' << '\xBF' << address; break;
    case 32: *this = address; break;
    case 64: throw 0;
    }
}
...
void Register::copy_zeroing(Register& other)
{
    ...
    case 8:
    case 16: throw 0;
    case 32: cpu << '\x0F' << '\xB6' << mod_rm(code, REG, other.code); break;
    case 64: throw 0;
    }
}
...
void Register::operator=(MemAddress addr)
{
    ...
    case 8: cpu << '\x8A' << addr; break;
    case 16: cpu << '\x66' << '\x8B' << addr; break;
    case 32: cpu << '\x8B' << addr; break;
    case 64: throw 0;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\x3A' << mod_rm(code, REG, other.code); break;
    case 16: cpu << '\x66' << '\x3B' << mod_rm(code, REG, other.code); break;
    case 32: cpu << '\x3B' << mod_rm(code, REG, other.code); break;
    case 64: throw 0;
    }
}
...
void Register::operator==(MemAddress addr)
{
    addr.other = code;
    switch(getBitMode(addr)) {
    case 8: cpu << '\x3A' << addr; break;
    case 16: cpu << '\x66' << '\x3B' << addr; break;
    case 32: cpu << '\x3B' << addr; break;
    case 64: throw 0;
    }
}
...
{
    switch(getBitMode()) {
    case 8: cpu << '\x80' << mod_rm(EX_7, REG, code) << (byte)test; break;
    case 16: cpu << '\x66' << '\x80' << mod_rm(EX_7, REG, code) << (unsigned short)test; break;
    case 32: cpu << '\x81' << mod_rm(EX_7, REG, code) << test; break;
    case 64: throw 0;
    }
}

unsigned char setConditions[JumpTypeCount - 1] = {0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,
