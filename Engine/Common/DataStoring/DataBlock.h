...
static_assert(
    sizeof(short) == 2, "Short must be 2 bytes for datablocks to work accross the network");
...
#define NAMERESOLVERTEMPLATEINSTANTIATION(DType, TVALDEFINE) \
    template<>                                               \
    struct DataBlockNameResolver<DType> {                    \
        static const short TVal = TVALDEFINE;                \
    };
...
public:
    ...
    virtual DataBlockAll* AllocateNewFromThis() const
    {
        ...
        return static_cast<DataBlockAll*>(
            (new DataBlock<DBlockT>(const_cast<const DataBlock<DBlockT>&>(*this))));
    }
...
#define TVALRESOLVERTYPE(BlockTypeT, DEFINEDValT)                   \
    template<>                                                      \
    struct TvalToTypeResolver<DEFINEDValT> {                        \
        static const BlockTypeT* Conversion(const DataBlockAll* bl) \
        {                                                           \
            return static_cast<const BlockTypeT*>(bl);              \
        };                                                          \
        static BlockTypeT* Conversion(DataBlockAll* bl)             \
        {                                                           \
            return static_cast<BlockTypeT*>(bl);                    \
        }                                                           \
    };
...
public:
    ...
    DLLEXPORT VariableBlock(const std::string& valuetoparse,
    ...
    inline bool ConvertAndAssingToVariable(ConvertT& var) const
    {
        ...
        // assign directly to the wanted value, should be faster than converting returning and
        // then assigning //
...
public:
    ...
    NamedVariableBlock(const char& var, const std::string& name) :
        VariableBlock(var), Name(name)
    {}
    NamedVariableBlock(void* var, const std::string& name) : VariableBlock(var), Name(name) {}
...
DLLEXPORT std::ostream& operator<<(
    std::ostream& stream, const Leviathan::VariableBlock& value);


// conversion template specifications //
#define CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(                          \
    BlockTypeName, ToConvertTypeName, ConvertActionToDo)                      \
    template<>                                                                \
    class DataBlockConverter<BlockTypeName, ToConvertTypeName> {              \
    public:                                                                   \
...
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, std::string, Convert::Utf16ToUtf8(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, bool, Convert::WstringFromBoolToInt(*block->Value) != 0);
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, float, Convert::WstringTo<float>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, double, Convert::WstringTo<double>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, char, (char)Convert::WstringTo<wchar_t>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    WstringBlock, int, Convert::WstringTo<int>(*block->Value));
//// ------------------ StringBlock conversions ------------------ //
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(StringBlock, std::string, (*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, std::wstring, Convert::Utf8ToUtf16(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, bool, Convert::StringFromBoolToInt(*block->Value) != 0);
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, float, Convert::StringTo<float>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, double, Convert::StringTo<double>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, char, Convert::StringTo<char>(*block->Value));
CONVERSIONTEMPLATESPECIFICATIONFORDATABLOCK(
    StringBlock, int, Convert::StringTo<int>(*block->Value));
