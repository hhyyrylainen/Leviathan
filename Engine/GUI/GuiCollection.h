#ifndef LEVIATHAN_GUICOLLECTION
#define LEVIATHAN_GUICOLLECTION
// ------------------------------------ //
#ifndef LEVIATHAN_DEFINE
#include "Define.h"
#endif
// ------------------------------------ //
// ---- includes ---- //
#include "Input\Key.h"
#include "Rocket\Core\ElementDocument.h"
#include "Script\ScriptScript.h"
#include "ObjectFiles\ObjectFileObject.h"

namespace Leviathan{ namespace Gui{

	// usability type define //
	typedef Key<OIS::KeyCode> GKey;

	class GuiManager;

	// this is used to pass Rocket pages to collections //
	class GuiLoadedSheet{
	public:
		GuiLoadedSheet(Rocket::Core::Context* context, const string &documentfile);
		~GuiLoadedSheet();

		DLLEXPORT Rocket::Core::Element* GetElementByID(const string &id);

	private:
		// used for finding by ID //
		Rocket::Core::ElementDocument* Document;
	};


	class GuiCollection : public Object{
	public:
		GuiCollection(const wstring &name, shared_ptr<GuiLoadedSheet> sheet, int id, const wstring &toggle, bool strict = false, 
			bool enabled = true);
		~GuiCollection();

		DLLEXPORT void UpdateState(bool newstate);
		DLLEXPORT inline void ToggleState(){
			UpdateState(!Enabled);
		}

		DLLEXPORT inline const GKey& GetTogglingKey(){
			return Toggle;
		}
		DLLEXPORT inline int GetID(){
			return ID;
		}
		DLLEXPORT inline const wstring& GetName(){
			return Name;
		}

		DLLEXPORT static bool LoadCollection(GuiManager* gui, const ObjectFileObject &data, shared_ptr<GuiLoadedSheet> sheet);
	private:
		wstring Name;
		int ID;
		bool Enabled;
		bool Strict;

		GKey Toggle;

		shared_ptr<ScriptScript> Scripting;
		shared_ptr<GuiLoadedSheet> ContainedInSheet;
	};

}}
#endif