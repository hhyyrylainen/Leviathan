...
#define LEVIATHAN_LEVIATHANJAVASCRIPTASYNC
// ------------------------------------ //
...
namespace Leviathan{ namespace Gui{
	...
	//! \brief Provides client applications and modules (not done) support for defining custom request responses
	//! \note This object should be directly passed to GlobalCEFHandler::RegisterCustomJavaScriptQueryHandler
	class JSAsyncCustom{
	public:
		...
		//! \param caller Is the LeviathanJavaScriptAsync object associated with a View that has called this function (these should not be mixed)
		//! \warning If caller is different the ID of different queries can be the same, so be sure to store both the caller and the id to uniquely
		//! identify queries
		DLLEXPORT virtual bool ProcessQuery(LeviathanJavaScriptAsync* caller, const CefString &request, int64 queryid, bool persists, 
			CefRefPtr<Callback> &callback) = 0;

		//! \brief Called if a query is canceled, nothing should be called after this
		//! \note This should work so that the waiting matching query is canceled and not responded to
		DLLEXPORT virtual void CancelQuery(LeviathanJavaScriptAsync* caller, int64 queryid) = 0;


		//! \brief Called when JSAsyncCustom is unconnected from a LeviathanJavaScriptAsync and this should CancelQuery all queries associated with
		//! that object
	...
	class LeviathanJavaScriptAsync : public CefMessageRouterBrowserSide::Handler, public ThreadSafe{
	public:
		...
		DLLEXPORT void RegisterNewCustom(JSAsyncCustom* newhandler);
		...
		DLLEXPORT inline bool _VerifyJSAccess(VIEW_SECURITYLEVEL minimumlevel, CefRefPtr<Callback> &callback){
			...
			callback->Failure(403, "View doesn't have a high enough security level to access this feature");
			return true;
		}
	...
	protected:
		...
		std::vector<JSAsyncCustom*> RegisteredCustomHandlers;
		View* Owner;
	};
