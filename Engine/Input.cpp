...
bool Input::Update(){
	...
	Engine::GetEngine()->GetWindow()->GetRelativeMouse(MouseX, MouseY);
	// get mouse move amount //
	if(CaptureMouse){
		// get distance to center of window //
		int width = Engine::GetEngine()->GetWindow()->GetWidth();
		int height = Engine::GetEngine()->GetWindow()->GetHeight();
		...
		// set to center //
		Engine::GetEngine()->GetWindow()->SetMouseToCenter();
	}
