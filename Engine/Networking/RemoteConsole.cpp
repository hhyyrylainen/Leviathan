...
DLLEXPORT Leviathan::RemoteConsole::RemoteConsole() :
    CloseIfNoRemoteConsole(false), CanClose(false)
{}
...
DLLEXPORT void Leviathan::RemoteConsole::UpdateStatus()
{
    ...
    for(auto iter = RemoteConsoleConnections.begin();
        iter != RemoteConsoleConnections.end();) {
    ...
    const MillisecondDuration& timeout /*= std::chrono::seconds(30)*/)
{
    ...
    AwaitingConnections.push_back(shared_ptr<RemoteConsoleExpect>(
        new RemoteConsoleExpect(assignname, SessionToken, onlylocalhost, timeout)));
}
// ------------------------------------ //
DLLEXPORT bool Leviathan::RemoteConsole::CanOpenNewConnection(
    std::shared_ptr<Connection> connection, std::shared_ptr<NetworkRequest> request)
{
    ...
    case NETWORK_REQUEST_TYPE::RemoteConsoleOpen: {
        ...
        LOG_WARNING("RemoteConsole: no matching awaiting connection found");
        return false;
    }
...
DLLEXPORT void Leviathan::RemoteConsole::OfferConnectionTo(
    std::shared_ptr<Connection> connectiontouse, const std::string& connectionname, int token)
{
    GUARD_LOCK();

    // Add to the expected connections //
    AwaitingConnections.push_back(
        shared_ptr<RemoteConsoleExpect>(new RemoteConsoleExpect(connectionname, token,
            connectiontouse->IsTargetHostLocalhost(), std::chrono::seconds(15))));

    // Send a request that the target connects to us //
    connectiontouse->SendPacketToConnection(
        std::make_shared<RequestDoRemoteConsoleOpen>(token), RECEIVE_GUARANTEE::Critical);
}
// ------------------------------------ //
DLLEXPORT void Leviathan::RemoteConsole::HandleRemoteConsoleRequestPacket(
    std::shared_ptr<NetworkRequest> request, std::shared_ptr<Connection> connection)
{
    // First check if it should be handled by CanOpenNewConnection which handless
    // all open connection packets //
    if(request->GetType() == NETWORK_REQUEST_TYPE::DoRemoteConsoleOpen ||
        request->GetType() == NETWORK_REQUEST_TYPE::RemoteConsoleOpen) {
    ...
    case NETWORK_REQUEST_TYPE::CloseRemoteConsole: {
        ...
        connection->SendPacketToConnection(
            std::make_shared<ResponseNone>(
                NETWORK_RESPONSE_TYPE::RemoteConsoleClosed, request->GetIDForResponse()),
            RECEIVE_GUARANTEE::Critical);
        return;
    }
    default: LOG_ERROR("RemoteConsole unhandled request type");
    }
...
DLLEXPORT void Leviathan::RemoteConsole::HandleRemoteConsoleResponse(
    ...
    std::shared_ptr<NetworkRequest> potentialrequest)
{
    ...
    case NETWORK_RESPONSE_TYPE::RemoteConsoleClosed: {
        ...
        Logger::Get()->Warning(
            "RemoteConsole: HandleRemoteConsoleResponse: got a "
...
Leviathan::RemoteConsole::RemoteConsoleExpect::RemoteConsoleExpect(const std::string& name,
    ...
    ConnectionName(name),
    SessionToken(token), OnlyLocalhost(onlylocalhost),
...
Leviathan::RemoteConsoleSession::~RemoteConsoleSession()
{
    ...
    if(CorrespondingConnection->IsValidForSend()) {
        ...
        CorrespondingConnection->SendPacketToConnection(
            std::make_shared<RequestNone>(NETWORK_REQUEST_TYPE::CloseRemoteConsole),
            RECEIVE_GUARANTEE::Critical);
    }
}
// ------------------------------------ //
