...
// This function is called every time for a collision point between the materials. This is recommended to be used for playing sound effects. //
...
namespace Leviathan{
	...
	struct PhysMaterialDataPair{
		...
		// Sets the softness of the material pair (Recommended 1.f and below, default 0.15f, higher value is less soft) //
		DLLEXPORT inline PhysMaterialDataPair& SetSoftness(const float &softness){
			Softness = softness;
			return *this;
		}
		// Sets the elasticity (restitution) of the material pair (Recommended 1.f and below, default 0.4f, higher value might be more elasticity) //
		DLLEXPORT inline PhysMaterialDataPair& SetElasticity(const float &elasticity){
			Elasticity = elasticity;
			return *this;
		}
		// Sets the friction between the materials (default static is 0.9f and dynamic (sliding) 0.5f) //
