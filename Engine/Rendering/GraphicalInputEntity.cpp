...
//! \brief Used to hold objects that are required for clearing a GraphicalInputEntity each
//! frame
class GEntityAutoClearResources {
public:
    GEntityAutoClearResources(Ogre::Root* destroyer) : Root(destroyer) {}
    ...
    ID(IDFactory::GetID())
{
    ...
    if(windowsafter == 1) {
        ...
        while(!FirstCEGUIRenderer) {
            ...
            Logger::Get()->Info(
                "GraphicalInputEntity: waiting for first window to initialize");
...
DLLEXPORT void Leviathan::GraphicalInputEntity::CreateAutoClearWorkspaceDefIfNotAlready()
{
    ...
    auto templatedworkspace = manager->addWorkspaceDefinition(CLEAR_WORKSPACE_NAME);
    ...
    rendernode->addTextureSourceName(
        "renderwindow", 0, Ogre::TextureDefinitionBase::TEXTURE_INPUT);
    ...
    Ogre::CompositorPassClearDef* clearpass =
        static_cast<Ogre::CompositorPassClearDef*>(targetpasses->addPass(Ogre::PASS_CLEAR));
...
DLLEXPORT void Leviathan::GraphicalInputEntity::SetAutoClearing(const string& skyboxmaterial)
{
    ...
    AutoClearResources->WorldsScene =
        ogre->createSceneManager(Ogre::ST_GENERIC, 1, Ogre::INSTANCING_CULLING_SINGLETHREAD,
    ...
    AutoClearResources->WorldWorkspace = ogre->getCompositorManager2()->addWorkspace(
        AutoClearResources->WorldsScene, GetOgreWindow(), AutoClearResources->WorldSceneCamera,
    ...
    try {
        ...
        Logger::Get()->Error(
            "GraphicalInputEntity: setting auto clear skybox " + e.getFullDescription());
    }
}
