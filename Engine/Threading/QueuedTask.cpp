...
DLLEXPORT Leviathan::ConditionalDelayedTask::ConditionalDelayedTask(
    ...
    QueuedTask(functorun),
    TaskCheckingFunc(canberuncheck), CheckingTime(Time::GetCurrentTimePoint() + delaytime),
    DelayBetweenChecks(delaytime)
{}
    ...
    const QueuedTaskCheckValues* const checkvalues)
{
    ...
    CheckingTime = Time::GetCurrentTimePoint() + DelayBetweenChecks;
...
DLLEXPORT Leviathan::DelayedTask::DelayedTask(
    std::function<void()> functorun, const MicrosecondDuration& delaytime) :
    QueuedTask(functorun),
    ExecutionTime(Time::GetCurrentTimePoint() + delaytime)
{}

DLLEXPORT Leviathan::DelayedTask::DelayedTask(
    std::function<void()> functorun, const WantedClockType::time_point& executetime) :
    QueuedTask(functorun),
...
DLLEXPORT Leviathan::RepeatingDelayedTask::RepeatingDelayedTask(
    ...
    DelayedTask(functorun, bothdelays),
    TimeBetweenExecutions(bothdelays), ShouldRunAgain(true)
{}

DLLEXPORT Leviathan::RepeatingDelayedTask::RepeatingDelayedTask(
    std::function<void()> functorun, const MicrosecondDuration& initialdelay,
    const MicrosecondDuration& followingduration) :
    DelayedTask(functorun, initialdelay),
    TimeBetweenExecutions(followingduration), ShouldRunAgain(true)
{}
...
void Leviathan::RepeatingDelayedTask::_PostFunctionRun()
{
    ...
    ExecutionTime = Time::GetCurrentTimePoint() + TimeBetweenExecutions;
}
// ------------------ RepeatCountedTask ------------------ //
DLLEXPORT Leviathan::RepeatCountedTask::RepeatCountedTask(
    std::function<void()> functorun, size_t repeatcount) :
    QueuedTask(functorun),
    MaxRepeats(repeatcount), RepeatedCount(0)
{}
...
DLLEXPORT Leviathan::RepeatCountedDelayedTask::RepeatCountedDelayedTask(
    std::function<void()> functorun, const MicrosecondDuration& bothdelays, int repeatcount) :
    RepeatCountedTask(functorun, repeatcount),
    TimeBetweenExecutions(bothdelays), ExecutionTime(Time::GetCurrentTimePoint() + bothdelays)
{}
...
DLLEXPORT Leviathan::RepeatCountedDelayedTask::RepeatCountedDelayedTask(
    ...
    RepeatCountedTask(functorun, repeatcount),
    TimeBetweenExecutions(followingduration),
    ExecutionTime(Time::GetCurrentTimePoint() + initialdelay)
{}
