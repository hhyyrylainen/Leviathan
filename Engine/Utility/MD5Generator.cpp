...
std::string MD5::hexdigest() const
{
  ...
  stringstream digeststring("");
  // set proper settings //
  digeststring << std::hex << std::setfill('0');


  //char buf[33];
  for (int i=0; i<16; i++)
    //sprintf(buf+i*2, "%02x", digest[i]);
    //sprintf_s(buf+i*2, sizeof(buf[0]),"%02x", digest[i]);
    digeststring << std::setw(2) << static_cast<unsigned>(digest[i]);
  //buf[32]=0;
