...
int main(int argcount, char* args[]){
	...
	for(size_t i = 0; i < cutpositions.size(); i += 2){
		...
		tokens.push_back(commandline.substr(cutpositions[i], cutpositions[i+1]-cutpositions[i]+1));
	}
	...
	if(tokens.size() != 3){
		...
		cout << "Invalid commandline, expected 3 arguments (got " << tokens.size() << ")" << endl;
		return -1;
	}
