...
TEST_CASE("Returning and Passing a custom script inherited class works with const", "[script]")
{
    PartialEngine<false> engine;
    ...
    REQUIRE(as->RegisterInterface("SomeInterface") >= 0);
    REQUIRE(as->RegisterInterfaceMethod("SomeInterface", "int GetValue() const") >= 0);
    ...
    auto sourcecode = std::make_shared<ScriptSourceFileData>("Script.cpp", __LINE__ + 1,
        ...
        "return SomeClassName(value);\n"
        "}\n"
        "int DoSomethingWithObject(const SomeClassName@ obj){\n"
        "return obj.GetValue();\n"
        "}\n");
