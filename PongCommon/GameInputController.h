...
enum PONG_INPUT_FLAGS {
    PONG_INPUT_FLAGS_LEFT = 0x1,
...
public:
    PongNInputter(
        int ownerid, int networkid, PlayerSlot* controlthis, PLAYERCONTROLS typetoreceive);
    ~PongNInputter();
...
protected:
    ...
    //! These actually don't have to be exactly synced between all clients, thus this is only
    //! sent in the full packet
...
public:
    ...
    std::map<int32_t, CONTROLKEYACTION>& MapControlsToKeyGrouping(PLAYERCONTROLS controls);
